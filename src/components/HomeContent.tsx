      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <RecentlyPlayed
          recentTracks={recentTracks?.map((track) => ({
            id: track.id,
            // Map 'name' (source) to 'title' (expected)
            title: track.name || "Unknown Title",
            // Map 'artist' (pass through)
            artist: track.artist,
            // Map 'image' or 'albumArt' (source) to 'coverUrl' (expected)
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar -mx-6 px-6">
              {madeForYou.map((item: any) => (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-2 mt-1">Made for you</p>
                </div>
              ))}
            </div>
          </section>
