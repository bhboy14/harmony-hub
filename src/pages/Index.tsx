...
const Index = () => {
  ...
  // Updated: Keep sidebar and now playing mutually exclusive
  useEffect(() => {
    ...
    <div className="min-h-screen bg-background flex overflow-hidden">
      ...
      <div className="flex-1 ml-[72px] flex flex-col h-screen">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto pb-32 no-scrollbar">
            ...
            {activeTab === "library" && (
              <div className="p-6 h-full animate-fade-in">
                <Tabs defaultValue="unified" className="space-y-6 h-full">
                  <TabsList className="bg-secondary">
                    <TabsTrigger
                      value="unified"
                      className="gap-2 data-[state=active]:bg-foreground data-[state=active]:text-background"
                    >
                      <Library className="h-4 w-4" /> Unified Library
                    </TabsTrigger>
                    <TabsTrigger
                      value="spotify"
                      className="gap-2 data-[state=active]:bg-foreground data-[state=active]:text-background"
                    ...
                    <TabsTrigger
                      ...
                      className="gap-2 data-[state=active]:bg-foreground data-[state=active]:text-background"
                    >
                      <HardDrive className="h-4 w-4" /> Local Files
                    </TabsTrigger>
                  </TabsList>

                  <div className="flex-1 overflow-hidden h-[calc(100vh-14rem)]">
                    <TabsContent value="unified" className="h-full mt-0">
                      <UnifiedLibrary />
                    </TabsContent>
                    <TabsContent value="spotify" className="h-full mt-0">
                      <SpotifyPlayer />
                    </TabsContent>
                    <TabsContent value="soundcloud" className="h-full mt-0">
                      <SoundCloudPlayer />
                    </TabsContent>
                    ...
                    <TabsContent value="local" className="h-full mt-0">
                      <MediaLibrary
                        {...mediaLibrary}
                        playTrack={(track) => unifiedAudio.playLocalTrack({ ...track, title: track.title })}
                        currentTrack={unifiedAudio.activeSource === "local" ? unifiedAudio.currentTrack : null}
                        ...
                        setVolume={unifiedAudio.setVolume}
                      />
                    </TabsContent>
                  </div>
                </Tabs>
            ...
            {activeTab === "azan" && (
              <div className="p-6 max-w-2xl mx-auto animate-fade-in h-full">
                <AzanPlayer
                  {...azanScheduler}
            ...
            {activeTab === "pa" && (
              <div className="p-6 max-w-4xl mx-auto animate-fade-in h-full">
                <PASystem />
              </div>
            )}
          ...
          {/* 4. Side-Hiding Now Playing Panel */}
          {/* This panel now pushes the content when open, but remains hidden otherwise */}
          {showNowPlaying && (
            <aside className="w-80 border-l border-white/10 bg-black/40 flex flex-col h-full animate-in slide-in-from-right-10 duration-300">
      ...
      {/* 5. Fixed Playback Bar (Always Bottom) */}
      <PlaybackBar />
    </div>
  );
};
